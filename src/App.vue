<template>
  <div class="min-h-screen bg-gray-50 p-6 flex justify-between">
    ...
    <div class="w-2/3 bg-white shadow-2xl rounded-lg p-8 relative">
      <h1 class="text-3xl font-bold text-blue-600 mb-6">Carrinho de Vendas</h1>
      ...
      <!-- Carrinho -->
      <div>
        <div v-if="carrinho.length" class="mb-4">
          ...
          <ul class="space-y-1 divide-y divide-gray-200 overflow-y-auto max-h-[calc(95vh-250px)]">
            ...
            >
              <button @click="removerDoCarrinho(index)" class="mr-4 text-red-500 hover:text-red-700">
              ...
              <span class="font-medium w-1/4 text-right">
                <button @click="diminuirQuantidade(index)" class="bg-red-500 text-white px-2 py-1 rounded-full">-</button>
                {{ produto.quantidade }}
                <button @click="aumentarQuantidade(index)" class="bg-green-500 text-white px-2 py-1 rounded-full">+</button>
              </span>
        ...
        <div v-else class="text-gray-600">O carrinho está vazio.</div>
      </div>
    </div>
  ...
  <div class="w-1/3 bg-white shadow-2xl rounded-lg p-8 ml-6 flex flex-col">
    <h2 class="text-2xl font-semibold text-blue-600 mb-4">Resumo da Compra</h2>
    <div class="mt-6 border-t border-gray-300 pt-4 flex-1">
      ...
      <hr>
      <div v-if="mostrarPesquisa" class="mb-6 flex items-center mt-4"> 
        <input 
          type="text" 
          v-model="cpf" 
          ref="inputCpf"
        ...
        />
        <button @click="adicionarClienteComFocus(cpf)" class="ml-4 bg-blue-600 text-white p-4 rounded-lg shadow-md">Pesquisar</button> 
      </div>
      <!-- Cliente -->
        <div v-if="cliente" class="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg flex items-center justify-between mt-4"> 
          <div>
            <p>Cliente: {{ cliente.nome }}</p> 
            <p>Frete: {{ formatarMoeda(cliente.taxa) }}</p>
          </div>
          <!-- Botão para alterar o cliente com ícone -->
          <button @click="alterarCliente" class="ml-4 bg-orange-600 text-white p-2 rounded-lg flex items-center">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-5 h-5 mr-2">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 3l4 4-10 10H7v-4L17 3z" />
            </svg>
            Alterar Cliente
          </button>
        </div>
    ...
    <div class="mt-auto text-left">
      <button @click="abrirModal" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-700 flex items-center space-x-2">
  ...
  <ModalFecharVenda :show="exibirModal" :carrinho="carrinho" :frete="parseFloat(frete)" :clienteId="cliente ? cliente.id : null"
  :clienteNome="cliente ? cliente.nome : null"   @close="fecharModal" @finalizar="enviarVenda"  :cashback="parseFloat(cashback)"/>
  </div>
</template>
